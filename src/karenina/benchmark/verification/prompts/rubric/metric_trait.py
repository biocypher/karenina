...
Canonical location for metric trait prompt building.
Re-exported by evaluators/rubric/prompts/metric_trait.py for backwards compatibility.
...
Format-specific content (JSON schema blocks, output format sections) is NOT
included here — it is injected by adapter instructions registered per-interface.
This keeps prompt builders format-agnostic.
"""
...
class MetricTraitPromptBuilder:
    ...
    Format-specific content (JSON schema, output format) is injected by adapter
    instructions, not included in the base prompts.
...
- Do NOT include duplicate items in the same list"""
    ...
    def build_user_prompt(
        ...
        if trait.evaluation_mode == "tp_only":
            return self._build_tp_only_user_prompt(question, answer, trait)
        else:
            return self._build_full_matrix_user_prompt(question, answer, trait)
