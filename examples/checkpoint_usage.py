#!/usr/bin/env python3
"""
Example usage of the Karenina checkpoint system.

This example demonstrates how to:
1. Create benchmarks programmatically
2. Add questions and answer templates
3. Save and load JSON-LD checkpoints
...
'''
    ...
    checkpoint.add_global_rubric_trait(
    ...
    checkpoint.add_question_rubric_trait(
        q2_id,
    ...
    # ========================================
    # 4. Save checkpoint to JSON-LD file
    # ========================================
    ...
    # ========================================
    # 5. Load checkpoint from file
    # ========================================
    print(f"\nLoading checkpoint from {output_path}...")
    loaded_checkpoint = Checkpoint.load(output_path)

    # Display loaded data
    print(f"Loaded benchmark: {loaded_checkpoint._checkpoint.name}")
    print(f"  Version: {loaded_checkpoint._checkpoint.version}")
    print(f"  Questions: {len(loaded_checkpoint.get_question_ids())}")

    global_rubric = loaded_checkpoint.get_global_rubric()
    if global_rubric:
    ...
    templates = loaded_checkpoint.get_finished_templates()
    ...
    results = loaded_checkpoint.run_verification(config)
    """)

    # ========================================
    # 8. Validate checkpoint
    # ========================================
    is_valid, msg = loaded_checkpoint.validate()
    print(f"\nCheckpoint validation: {'✅ Valid' if is_valid else '❌ Invalid'}")
    if not is_valid:
        print(f"  Error: {msg}")

    print("\n" + "=" * 50)
    print("Example complete! The checkpoint file can be:")
