...
|----------|-------------------|-------------------|
| [Benchmark Operations](benchmark-operations.md) | All | Full Benchmark API: creating, populating, templates, rubrics, readiness, filtering, collection protocols |
| [Factual QA Benchmark](factual-qa-benchmark.md) | Template-only | Hand-written templates: boolean check, string normalization, numeric tolerance, regex in `verify()`, partial credit |
...
| [Choosing Rubric Traits](choosing-rubric-traits.md) | Template + rubric | Need-driven trait selection: 7 evaluation needs mapped to the right trait type, decision flowchart |
| [Scaled Benchmark Creation](scaled-benchmark-creation.md) | Power user | Bulk ingestion, `generate_all_templates()`, `AnswerBuilder`, ADeLe classification, few-shot examples |
| [AI-Assisted Template Construction](ai-assisted-template-construction.md) | Template generation | `generate_answer_template()`, `AnswerBuilder`, two-phase generation, batch `generate_all_templates()` |
| [ADeLe Classification](adele-classification.md) | Question profiling | 18-dimension classification, `QuestionClassifier`, batch scoring, ADeLe rubrics, complexity filtering |
...
| Generate templates automatically | `benchmark.generate_all_templates(model, model_provider)` | [Scaled Benchmark Creation](scaled-benchmark-creation.md) |
| Build templates programmatically | `AnswerBuilder().add_attribute(...).compile()` | [Scaled Benchmark Creation](scaled-benchmark-creation.md) |
...
| Check readiness | `benchmark.check_readiness()` | [Benchmark Operations](benchmark-operations.md) |
| Filter questions | `benchmark.filter_questions(finished=True, has_template=True)` | [Benchmark Operations](benchmark-operations.md) |
