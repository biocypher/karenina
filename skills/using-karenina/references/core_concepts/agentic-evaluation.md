...
Three built-in adapters set `agent_tier="deep_agent"`: `claude_agent_sdk`, `langchain_deep_agents` (available only when the `deepagents` package is installed), and the AgentPort-only `omp` adapter. OMP runs its own tool loop and exposes it to Karenina through ACP v1. Native OMP is a host process rather than an OS sandbox; use external container isolation when a hard boundary is required. See [Sandboxed Runtime Backends](#10-sandboxed-runtime-backends).
