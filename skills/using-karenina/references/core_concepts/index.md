...
| **Verification Pipeline** | The 13-stage engine (with sub-stages 7a/7b and 11a/11b) that executes evaluation end to end | [Verification Pipeline](verification-pipeline.md) |
...
| **Extending Runs** | Reuse prior traces to add judges/answerers/replicates (`extend_template`) or attach a new rubric (`extend_rubric`) | [Extending Runs](extending-runs.md) |
...
| **Few-Shot** | Example injection for improved LLM response accuracy | [Few-Shot](few-shot.md) |
...
[Read more about few-shot examples →](few-shot.md)
