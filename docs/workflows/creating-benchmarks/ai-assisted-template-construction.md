...
`generate_answer_template()` takes a `Question` object and returns Python code for an `Answer(BaseAnswer)` class. The generation makes three sequential LLM calls. First it plans the field design and identifies scope and ambiguity. Second it converts the plan into bounded fields and reference values. Third it writes the field instructions that tell the judge what to extract from responses. Karenina then assembles and structurally validates the Python template.
...
The three calls happen automatically inside `generate_answer_template()`. You never call these stages separately. The function returns the final Python code string ready for review and use.
...
On each invocation the `progress_callback` receives a public `TemplateProgressEvent`, importable from `karenina.benchmark`. Read fields like `event.percentage` (0 to 100), `event.event` (the event type, such as `task_completed`), and the per-question counts to display progress in scripts or the GUI. For production use, pass a real model and provider. The function calls `generate_answer_template()` internally for each question.
...
- [Factual QA Benchmark](factual-qa-benchmark.md): Hand-written template patterns (boolean, string, numeric, regex)
- [Scaled Benchmark Creation](scaled-benchmark-creation.md): Bulk ingestion, ADeLe classification, and few-shot examples
