...
class Answer(BaseAnswer):
    gene_symbol: str = Field(
        description=(
            "The official HGNC gene symbol mentioned in the response, in uppercase "
            "without hyphens (e.g., 'TP53' not 'p53'). If the response uses an alias "
...
class Answer(BaseAnswer):
    temperature: float = Field(
        description=(
            "The boiling point temperature stated in the response, in degrees Celsius. "
            "If the response gives the value in Fahrenheit or Kelvin, extract the "
            "Celsius equivalent."
        )
    )
...
class Answer(BaseAnswer):
    population: int = Field(
        description=(
            "The estimated population of the city as stated in the response, "
            "as a whole number. If the response gives an approximate figure "
            "(e.g., 'about 8 million'), extract the numeric value (8000000)."
        )
    )
...
!!! tip "Consider boolean decomposition"
    If you have a known, fixed set of expected items, [boolean decomposition](../core_concepts/answer-templates.md#lists-vs-boolean-decomposition) is often simpler — each item becomes its own `bool` field, eliminating string normalization and giving per-item partial credit automatically.
...
class Answer(BaseAnswer):
    symptoms: list[str] = Field(
        description=(
            "All symptoms of the condition explicitly listed in the response. "
            "Use lowercase common names (e.g., 'fever' not 'pyrexia'). Include "
            "only symptoms the response attributes to this specific condition, "
            "not symptoms mentioned in other contexts."
        )
    )
...
!!! tip "Consider boolean decomposition"
    For a known set of required items, individual `bool` fields (one per required item) avoid string normalization entirely. See the [Boolean Attribute Pattern](#boolean-attribute-pattern) below or the [core concepts guide](../core_concepts/answer-templates.md#lists-vs-boolean-decomposition).
...
class Answer(BaseAnswer):
    proteins: list[str] = Field(
        description=(
            "All proteins the response explicitly names as part of the signaling "
            "pathway. Use standard gene symbols in uppercase (e.g., 'EGFR', 'RAS'). "
            "Include only proteins the response associates with the pathway, not "
            "proteins mentioned in other contexts."
        )
    )
...
class Answer(BaseAnswer):
    drug_name: str = Field(
        description=(
            "The generic (non-brand) name of the drug mentioned in the response, "
            "in lowercase (e.g., 'venetoclax' not 'Venclexta')."
        )
    )
    target: str = Field(
        description=(
    ...
    mechanism: str = Field(
        description=(
            "The mechanism of action described in the response, as a single lowercase "
            "word (e.g., 'inhibitor', 'agonist', 'antagonist'). If the response uses "
            "a phrase like 'blocks the activity of', normalize to the standard term."
        )
    )
...
For rigorous evaluation, use boolean fields that check for concept presence rather than extracting text. This avoids string matching pitfalls entirely. This approach is especially powerful as a **list decomposition** strategy — instead of extracting a `list[str]` and normalizing, use one `bool` field per expected item for unambiguous evaluation and automatic per-item partial credit:
...
class Answer(BaseAnswer):
    mentions_bcl2: bool = Field(
        description=(
            ...
            "B-cell lymphoma 2) as the pharmacological target of the drug. False "
            "if BCL2 is mentioned only as a pathway member or not at all."
        )
    )
    ...
    mentions_apoptosis: bool = Field(
        description=(
            "True if the response mentions apoptosis, programmed cell death, or "
            "apoptotic pathway as a consequence of the drug's action. False otherwise."
        )
    )
...
template_code = '''class Answer(BaseAnswer):
    target: str = Field(
        description=(
            "The protein target of the drug as stated in the response. "
            "Use the standard gene symbol in uppercase without hyphens "
            "(e.g., 'BCL2' not 'Bcl-2')."
        )
    )
...
count_template = '''class Answer(BaseAnswer):
    count: int = Field(
        description=(
            "The number of chromosomes stated in the response, as a whole "
            "number. If the response describes pairs, extract the total count "
            "(e.g., '23 pairs' should be extracted as 46)."
        )
    )
...
**Write clear field descriptions.** The Judge LLM only sees the field name, type, and description — your description is its only instruction. For each field, specify what to extract, the expected format (case, notation, units), scope boundaries (what counts and what doesn't), and how to handle ambiguity. See [Writing Good Field Descriptions](../core_concepts/answer-templates.md#writing-good-field-descriptions) for detailed guidance and examples.
