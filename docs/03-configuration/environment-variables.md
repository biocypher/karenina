...
The `karenina init` command creates a `.env` template with placeholder entries for all three main providers. See [Workspace Initialization](workspace-init.md) for details.
...
- [Workspace Initialization](workspace-init.md) — `karenina init` creates default directories and `.env` template
