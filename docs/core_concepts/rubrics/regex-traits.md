...
Use `RegexRubricTrait` when the evaluation is genuinely about the presence or absence of a literal text pattern. If the check requires semantic interpretation, prefer [LLM traits](llm-traits.md). If the check is deterministic but more complex than a regex match, prefer [Callable traits](callable-traits.md).
