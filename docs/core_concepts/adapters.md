...
6. Call `register_adapter(adapter)` so the registry can run `aclose()` at shutdown (see [Adapter Lifecycle and Cleanup Tracking](../advanced-adapters/writing-adapters.md#adapter-lifecycle-and-cleanup-tracking))
