...
Validates that adapters with agent_tier="deep_agent" correctly configure
...
- Deep agent adapters have built-in filesystem tools (ls, glob, read_file)
...
class TestWorkspaceAccessConformance:
    """Verify that deep agent adapters can access real workspace files."""

    def test_deep_agent_adapters_must_not_use_virtual_backend(
        self, all_registered_interfaces, mock_model_config_for_interface
    ):
        """Deep agent adapters must configure real filesystem access.
