...
@pytest.mark.unit
class TestValidateTemplateWithVerificationSystem:
    """Tests for validate_template_with_verification_system method."""

    def test_validate_nonexistent_question(self) -> None:
        """Test validating template for nonexistent question."""
        benchmark = Benchmark.create(name="test")
        manager = TemplateManager(benchmark)

        valid, error = manager.validate_template_with_verification_system("nonexistent")

        assert valid is False
        assert "Question not found" in error

    def test_validate_no_template(self) -> None:
        """Test validating when question has only default template."""
        benchmark = Benchmark.create(name="test")
        manager = TemplateManager(benchmark)

        q_id = benchmark.add_question("Q?", "A")

        valid, error = manager.validate_template_with_verification_system(q_id)

        # Questions get a default template, so validation passes
        # But the default template may not be valid for verification
...
class TestGetTemplateStatistics:
    """Tests for get_template_statistics method."""
    ...
    def test_statistics_empty(self) -> None:
        ...
        assert stats["templates_with_errors"] == 0

    def test_statistics_with_templates(self) -> None:
...
'''
        ...
        assert stats["avg_template_length"] > 0
        assert stats["min_template_length"] > 0
        assert stats["max_template_length"] >= stats["min_template_length"]

    def test_statistics_counts_errors(self) -> None:
        """Test that statistics include template errors."""
        benchmark = Benchmark.create(name="test")
        manager = TemplateManager(benchmark)

        q_id = benchmark.add_question("Q?", "A")

        # Add valid template then corrupt it
        manager.add_answer_template(q_id, VALID_TEMPLATE)
        benchmark._questions_cache[q_id]["answer_template"] = "invalid python code {"

        stats = manager.get_template_statistics()

        assert stats["templates_with_errors"] == 1
