...
class TestTaskEvalManualTraits:
    ...
    def test_evaluate_with_manual_traits_only(
        ...
        # Check that manual traits were evaluated at the step level (not question level)
        # Standalone rubrics are evaluated once for all questions and stored in rubric_scores
        rubric_scores = result.global_eval.rubric_scores
        assert rubric_scores["has_code"] is True  # Contains code block
        assert rubric_scores["mentions_python"] is True  # Mentions Python
        assert rubric_scores["short_response"] is True  # Less than 20 words (inverted result of word_count_over_20)
    ...
    def test_evaluate_with_mixed_traits(
        ...
        # Check that both trait types were evaluated at the step level
        # Standalone rubrics (both LLM and manual) are evaluated once for all questions
        rubric_scores = result.global_eval.rubric_scores

        # Manual traits
        assert rubric_scores["mentions_def"] is True  # Contains "def"
        assert rubric_scores["long_explanation"] is False  # Less than 20 words

        # LLM traits
        assert rubric_scores["accuracy"] is True
        assert rubric_scores["clarity"] == 4
    ...
    def test_step_evaluation_with_manual_traits(
        ...
        # Standalone rubrics are evaluated at the step level, not question level
        rubric_scores = result.per_step["step1"].rubric_scores
    ...
    def test_fallback_evaluation_with_manual_traits(self, verification_config, sample_callable_registry) -> None:
        ...
        # Check that manual traits were evaluated in fallback mode at step level
        rubric_scores = result.global_eval.rubric_scores
    ...
    def test_error_handling_with_manual_traits(self, verification_config) -> None:
        ...
        manual_traits = [
            ...
            ManualRubricTrait(name="valid_pattern", pattern=r"[Tt]est"),  # Case-insensitive pattern
        ]
        ...
        # Check that error was handled gracefully at step level
        rubric_scores = result.global_eval.rubric_scores
    ...
    def test_global_and_step_manual_traits_together(self, verification_config, sample_callable_registry) -> None:
        ...
        # Check global evaluation - standalone rubrics are at step level
        global_rubric_result = result.global_eval.rubric_scores
        assert global_rubric_result["mentions_python_global"] is True

        # Check step evaluation - standalone rubrics are at step level
        step_rubric_result = result.per_step["coding"].rubric_scores
